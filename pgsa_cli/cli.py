...
from core.metrics.drift import drift_report, write_drift_report
from core.supervisor.context import continuation_prompt, export_context, read_session_harness, record_session_event
from core.templates.initializer import init_pgsa
from core.validators.artifacts import issues_to_dicts, validate


def cmd_init(args: argparse.Namespace) -> int:
    written = init_pgsa(Path(args.root), force=args.force)
...
def cmd_watch(args: argparse.Namespace) -> int:
    ...
    payload = {
        "watch_mode": "one_shot",
        "note": "v0.1 watch is a convenience status check, not a background supervisor.",
        "valid": not any(issue.severity == "error" for issue in issues),
...
3. Revert the contract or decision.
