...
This file may be generated by the optional CLI or created/maintained by the
session agent itself after reading `pgsa/sessions.yaml`. In both cases, the
session identity comes from the registry, not from this folder.
...
If this folder was created by the running agent, keep `AGENTS.md`, `SKILL.md`,
and `PGSA_SESSION.json` aligned with the current `pgsa/sessions.yaml` entry.
When the session registration changes, update these launch files rather than
...
Use project-level accepted skill provenance from:

`<pgsa_root>/skills/signed_skill_manifest.yaml`

Only use entries whose `applies_to` contains `{session_id}` or `*`. Do not load
raw `<pgsa_root>/imports/sources/<source_id>/` skill instructions unless this
session is `external_import_review` or that source is explicitly listed in this
session's `must_read`.
"""
